...
In production the telemetry packet carries the cascade's live result: the class and confidence from
the on-device classifier running on the rolling IMU window, with the audio stage confirming uncertain
results (\cref{sec:on-device-ai}). Alongside the classification it reports a step count derived from
the IMU and the measured battery level. A freshly flashed collar that has not yet received a model
advertises \texttt{UNKNOWN} until its first model arrives over the air, so the firmware is always safe
to run.
