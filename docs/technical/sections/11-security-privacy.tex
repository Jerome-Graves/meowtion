...
\subsection{Link encryption}

The collar-to-station Bluetooth link is encrypted. Both the audio stream and the over-the-air model
transfer require an encrypted connection (LE Secure Connections), established fresh on each connection so
a battery collar that reboots carries no stale pairing state. A nearby device that has not paired cannot
subscribe to the audio stream or write a model to the collar, so it can neither eavesdrop on the link nor
overwrite the model flash; an over-the-air image is in any case committed only after a CRC-32 integrity
check (\cref{sec:training-pipeline}). Authenticating the pairing against an active man-in-the-middle, with
a key provisioned over USB, is future work (\cref{sec:future-work}).

\subsection{Static analysis}

Every push and pull request runs CodeQL static analysis (GitHub code scanning), with findings surfaced in
the repository's security tab; dependency updates are raised automatically, and the CI workflow actions
are pinned to commit hashes and do not persist credentials in the checkout.
