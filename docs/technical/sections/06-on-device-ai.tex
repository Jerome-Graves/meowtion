...
delivered over the air (\cref{sec:firmware-collar,sec:protocols}); a re-trained model is therefore
swapped in without reflashing. The engine is also safe with no model present, the presence queries
return false and the cascade returns \texttt{UNKNOWN} rather than a wrong answer, which is the state a
freshly flashed collar is in before its first model arrives.
